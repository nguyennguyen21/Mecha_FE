...
interface ProfileFormData {
  ...
  background: string; // 👈 dùng làm background color
  audio: string;
  audioImage: string;
  audioTitle: string;
  customCursor: string;
...
const AudioProfile: React.FC<AudioProfileProps> = ({
  ...
  return (
    <div
      className="rounded-3xl shadow-xl border border-gray-200 dark:border-gray-700 p-8 transition-all duration-300 hover:shadow-2xl"
      style={{ backgroundColor: formData.background || "#ffffff" }} // 👈 lấy màu từ formData.background
    >
      {/* Header */}
            ...
            {uploadingFiles.audioImage && (
              <div className="absolute inset-0 bg-black/70 rounded-2xl flex items-center justify-center animate-pulse">
                <div className="flex flex-col items-center text-white">
                  ...
                  <p className="text-sm mt-3 font-medium opacity-90">
                    Uploading Cover...
                  </p>
                </div>
              </div>
